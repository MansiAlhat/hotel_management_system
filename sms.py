...
def update_data():
    Query = 'update hotel set name=%s,mobile=%s,email=%s,address=%s,gender=%s,dob=%s where id=%s'
...
def search_data():
    Query = 'select * from hotel where id=%s or name=%s or email=%s or mobile=%s or address=%s or gender=%s or dob=%s'
...
def add_data():
    if idEntry.get() =='' or nameEntry.get()=='' or phoneEntry.get()=='' or emailEntry.get()=='' or addressEntry.get()=='' or genderEntry.get()=='' or dobEntry.get()=='':
        ...
        try:
            Query = 'insert into hotel values(%s,%s,%s,%s,%s,%s,%s)'
...
def connect_database():
    def connect():
        ...
        try:
            Query = 'create database hotelmanagementsystem'
            mycursor.execute(Query)

            Query = 'use hotelmanagementsystem'
            mycursor.execute(Query)

            Query = 'create table hotel (id int not null primary key , name varchar(30),mobile varchar(10),email varchar(30),address varchar(100),gender varchar(20),dob varchar(50))'
            mycursor.execute(Query)
...
logo_image = PhotoImage(file='hotel.png')
logo_Label = Label(leftFrame,image=logo_image)
logo_Label.grid(row=0,column=0)

addstudentButton = ttk.Button(leftFrame,text='Add Customer',width=25,state=DISABLED,command=lambda :toplevel_data('Add Student','Add',add_data))
addstudentButton.grid(row=1,column=0,pady=20)

searchstudentButton = ttk.Button(leftFrame,text='Search Customer',width=25,state=DISABLED,command=lambda :toplevel_data('Search Student','Search',search_data))
searchstudentButton.grid(row=2,column=0,pady=20)

deletestudentButton = ttk.Button(leftFrame,text='Delete Customer',width=25,state=DISABLED,command=delete_student)
deletestudentButton.grid(row=3,column=0,pady=20)

updatestudentButton = ttk.Button(leftFrame,text='Update Customer',width=25,state=DISABLED,command=lambda :toplevel_data('Update Student','Updata',update_data))
updatestudentButton.grid(row=4,column=0,pady=20)

showstudentButton = ttk.Button(leftFrame,text='Show Customer',width=25,state=DISABLED,command=show_student)
showstudentButton.grid(row=5,column=0,pady=20)

exitButton = ttk.Button(leftFrame,text='Exit Customer',width=25,command=iexit)
...
studentTable.heading('ID',text='Customer No')
